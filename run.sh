...
sudo apt install python3-pip gnome-tweak-tool gnome-shell-extensions curl zsh
chsh -s $(which zsh)
...
sudo bash $DIRECTORY/Grub/install-vimix.sh
sudo cp $DIRECTORY/image/lockscreen.jpg $DIRECTORY/image/wallpaper.jpg /usr/share/backgrounds
...
sudo bash $DIRECTORY/Plymouth/install.sh
cp $DIRECTORY/.zshrc ~/
cp -r $DIRECTORY/vscode/.vscode ~/
